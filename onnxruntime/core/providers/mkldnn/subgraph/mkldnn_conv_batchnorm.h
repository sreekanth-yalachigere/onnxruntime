...
class MklDnnConvBatchNorm : public MklDnnKernel {
 public:
  ...
  Status CreatePrimitives(const OrtCustomOpApi* api,
    ...
    if (mklnode_ptr_->conv_bias == true) {
      ...
      int batchNormIndix = (mklnode_ptr_->conv_bias == true) ? input_index + 3 : input_index + 2;
  ...
  void DumpData(std::string message, const float* data, size_t size) {
	// for Debugging 
